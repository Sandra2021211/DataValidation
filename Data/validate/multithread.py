...
# Thread-safe row comparison function
def compare_row_chunk_threaded(chunk_df, source_indexed, mapping_data):
    mismatches = []
...
class FullValidator:
    ...
    def find_min_unique_key(self):
        # Hardcoding composite key: claim_id + claim_segment
    ...
    def compare_rows(self):
        ...
        # Split dest_df into chunks for threading
        num_threads = min(8, len(self.dest_df))
        chunks = np.array_split(self.dest_df, num_threads)

        mismatches = []
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            futures = [
                executor.submit(compare_row_chunk_threaded, chunk, source_indexed, self.mapping_data)
                for chunk in chunks
            ]
...
if __name__ == "__main__":
    validator = FullValidator("/workspaces/DataValidation/Data/src_data/insurance_claim.csv", "/workspaces/DataValidation/Data/src_data/cleaned_insurance_claim_.csv", "/workspaces/DataValidation/Data/src_data/insurance_mapping.json")
